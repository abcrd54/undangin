...
export const guest = (() => {
    ...
    const showGuestName = () => {
         ...
         * Ex. ulems.my.id/?alamat=address&to=name
         */
        ...
        if (alamat) {
            ...
            const template = `<p class="m-0 p-0" style="font-size: 0.95rem;">${util.escapeHtml(alamat)}</p>`;
            util.safeInnerHTML(div, template);

            guestAddress?.appendChild(div);
        }
    ...
    const buildGoogleCalendar = () => {
         ...
         * @returns {string}
         */
        const formatDate = (d) => (new Date(d.replace(' ', 'T'))).toISOString().replace(/[-:]/g, '').split('.').shift();

        const timeData = document.body.getAttribute('data-time');
        const url = new URL('https://calendar.google.com/calendar/render');
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'The Wedding of Farid and Icha',
            dates: `${formatDate(timeData)}/${formatDate(timeData)}`,
            details: 'Tanpa mengurangi rasa hormat, kami mengundang Anda untuk berkenan menghadiri acara pernikahan kami. Terima kasih atas perhatian dan doa restu Anda, yang menjadi kebahagiaan serta kehormatan besar bagi kami.',
            location: 'Robayan RT 22 RW 03, Kec. Kalinyamatan, Kab. Jepara.',
            ctz: config.get('tz'),
        });
